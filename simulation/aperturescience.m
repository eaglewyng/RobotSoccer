...
function aperturescience=controller_home_full_state(uu,P)
    ...
    v1 = play_bank_shot(robot(:,1), ball, P,t);
    v2 = play_screen_opponent_from_goal(robot(:,2), opponent(:,2), P);
...
function v = play_bank_shot(robot, ball, P,t)
  
    persistent play_state
  ...
  % normal vector from ball to goal
  n = [P.goal(1) ; (P.goal(2)+P.field_width)]-ball;
  n = n/norm(n);
  % compute position 10cm behind ball, but aligned with goal.
  position = ball - 0.2*n;
  ...
  switch play_state
    case 0
          if norm(position-robot(1:2))<.1,
              play_state = 1;
              v = skill_go_to_point_for_wall_shot(robot, [P.goal(1) ; (P.goal(2)+P.field_width)], P);
        %       v = skill_go_to_point_for_wall_shot(robot, position, P);
        
          else
              v = skill_go_to_point_for_wall_shot(robot, position, P);
          end
    case 1
      v = skill_go_to_point_for_wall_shot(robot, [P.goal(1) ; (P.goal(2)+P.field_width)]*.4, P);
    otherwise
      v = skill_go_to_point_for_wall_shot(robot, position, P);
   end
...
function v = play_screen_opponent_from_ball(robot, opponent, ball, P)
    n = opponent(1:2)-ball;
    n = n/norm(n);
    
    position = opponent(1:2) - .2*n;
    v = skill_go_to_point(robot, position,P);
end

function v = play_screen_opponent_from_goal(robot, opponent, P)
    n = opponent(1:2)- P.goal;
    n = n/norm(n);
...
function v = play_prepare_for_wall_shot(robot, ball, P)
  % normal vector from ball to goal
  n = P.goal-ball;
  n = n/norm(n);
  % compute position 10cm behind ball, but aligned with goal.
  ...
  if norm(position-robot(1:2))<.21,
      v = skill_go_to_point_for_wall_shot(robot, P.goal, P);
  else
      v = skill_go_to_point_for_wall_shot(robot, position, P);
  end
end
...
function v = skill_go_to_point_for_wall_shot(robot, point, P)
    ...
    % control angle to -pi/2
    theta_d = atan2((P.goal(2)+P.field_width)-robot(2), P.goal(1)-robot(1));
    omega = -P.control_k_phi*(robot(3) - theta_d); 
...
function v = utility_half_velocity(v,P)
    if v(1) >  P.robot_max_vx/2,    v(1) =  P.robot_max_vx/2;    end
    if v(1) < -P.robot_max_vx/2,    v(1) = -P.robot_max_vx/2;    end
    if v(2) >  P.robot_max_vy/2,    v(2) =  P.robot_max_vy/2;    end
    if v(2) < -P.robot_max_vy/2,    v(2) = -P.robot_max_vy/2;    end
    if v(3) >  P.robot_max_omega, v(3) =  P.robot_max_omega; end
    if v(3) < -P.robot_max_omega, v(3) = -P.robot_max_omega; end
end

function v = utility_no_velocity(v,P)
    if v(1) >  0,    v(1) =  0;    end
    if v(1) < 0,    v(1) = 0;    end
    if v(2) > 0,    v(2) =  0;    end
    if v(2) < 0,    v(2) = 0;    end
